...
import { X, ArrowDown, Paperclip } from "lucide-react";
import type { Drawing } from "@/data/ProjectDrawingsData";
        ...
        <div className="p-6 mt-2 border-t border-gray-300 bg-white">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <button className="flex items-center gap-2 md:px-4 px-2 py-1 bg-[#9CA3AF] hover:bg-[#E2E8F0] text-white rounded-full md:text-base text-sm font-medium transition-all">
              <a href={DrawingPlanImg} download="DrawingPlanImg.svg" className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-3 ml-auto flex-wrap">
              {drawing.status === "Approved" ? (
                <div className="flex items-center gap-2">
                  ...
                  <span onClick={onClose} className="px-5 py-1.5 bg-[#DCFCE7] text-[#16A34A] rounded-full text-sm font-medium border border-[#BBF7D0]">
                    Approved
                  </span>
                </div>
              ) : drawing.status === "Revision Required" ? (
                <span onClick={onClose} className="px-5 py-1.5 bg-[#FFF7ED] text-[#FF9409] rounded-full text-sm font-medium border border-[#FFEDD5]">
                  Sent for Revision
                </span>
              ) : (
                <>
                  <button onClick={() => setIsCancelModalOpen(true)} className="px-5 py-1.5 bg-[#FF9409] text-white rounded-full text-sm font-medium shadow-sm shadow-orange-100">
                    Revision Required
                  </button>
                  <button onClick={onClose} className="px-5 py-1.5 bg-[#3AB449]  text-white rounded-full text-sm font-normal shadow-sm shadow-green-100">
                    Approve
                  </button>
                </>
              )}
        ...
        >
          ...
          {drawing.status === "Pending Review" && (
            <div className="mt-6 flex items-center gap-2 border border-gray-200 rounded-xl p-2 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
              ...
              />
              <button onClick={onClose} className="p-2 text-[#00000080] hover:text-gray-600">
                <Paperclip className="w-5 h-5 -rotate-5" />
              </button>
              <Button onClick={()=>{
                onClose();
                onSendMessage();
              }} className="text-sm font-normal bg-[linear-gradient(90deg,#2563EB_0%,#4F46E5_100%)] hover:bg-blue-700 text-white rounded-lg flex items-center md:gap-2 gap-1 md:px-6 px-3 py-3 h-auto">
                Send Comment
              </Button>
            </div>
          )}
